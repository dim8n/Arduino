...
void display_strelka() {
  strelka_i = 1+seconds-int(seconds/30)*30; //lcd.print(strelka_i);
  
  if(strelka_i==1)  lcd.print("       <>       ");  // 1
  if(strelka_i==2)  lcd.print("      <<>>      ");  // 2
  if(strelka_i==3)  lcd.print("     <<<>>>     ");  // 3
  if(strelka_i==4)  lcd.print("    <<<<>>>>    ");  // 4
  if(strelka_i==5)  lcd.print("   <<<<<>>>>>   ");  // 5
  if(strelka_i==6)  lcd.print("  <<<<<<>>>>>>  ");  // 6
  if(strelka_i==7)  lcd.print(" <<<<<<<>>>>>>> ");  // 7
  if(strelka_i==8)  lcd.print("<<<<<<<<>>>>>>>>");  // 8
  if(strelka_i==9)  lcd.print("<<<<<<<  >>>>>>>");  // 9
  if(strelka_i==10) lcd.print("<<<<<<    >>>>>>");  // 10
  if(strelka_i==11) lcd.print("<<<<<      >>>>>");  // 11
  if(strelka_i==12) lcd.print("<<<<        >>>>");  // 12
  if(strelka_i==13) lcd.print("<<<          >>>");  // 13
  if(strelka_i==14) lcd.print("<<            >>");  // 14
  if(strelka_i==15) lcd.print("<              >");  // 15
  if(strelka_i==16) lcd.print(">              <");  // 16
  if(strelka_i==17) lcd.print(">>            <<");  // 17
  if(strelka_i==18) lcd.print(">>>          <<<");  // 18
  if(strelka_i==19) lcd.print(">>>>        <<<<");  // 19
  if(strelka_i==20) lcd.print(">>>>>      <<<<<");  // 20
  if(strelka_i==21) lcd.print(">>>>>>    <<<<<<");  // 21
  if(strelka_i==22) lcd.print(">>>>>>>  <<<<<<<");  // 22
  if(strelka_i==23) lcd.print(">>>>>>>><<<<<<<<");  // 23
  if(strelka_i==24) lcd.print(" >>>>>>><<<<<<< ");  // 24
  if(strelka_i==25) lcd.print("  >>>>>><<<<<<  ");  // 25
  if(strelka_i==26) lcd.print("   >>>>><<<<<   ");  // 26
  if(strelka_i==27) lcd.print("    >>>><<<<    ");  // 27
  if(strelka_i==28) lcd.print("     >>><<<     ");  // 28
  if(strelka_i==29) lcd.print("      >><<      ");  // 29
  if(strelka_i==30) lcd.print("       ><       ");  // 30
...
void setup() {
  ...
  stored_h = EEPROM.read(addr_h); stored_m = EEPROM.read(addr_m);
